...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['csv_file'])) {
    ...
    $percentageDevotionCommittee = isset($_POST['percentageDevotionCommittee']) ? $_POST['percentageDevotionCommittee']/100 : 0.1; //0.1;
    $percentageOldGroupMembership = isset($_POST['percentageOldGroupMembership']) ? $_POST['percentageOldGroupMembership']/100: 0.3; //0.3;
    ...
    foreach ($data as &$row) {
        ...
        if (!isset($groups[$old_group])) {
            $groups[$old_group] = ['total' => 0, 'devotion_committee' => 0, 'new_groups' => []];
        }
    ...
    foreach ($data as &$row) {
        ...
        $max_general_in_group = ceil($percentageOldGroupMembership * $groups[$old_group]['total']); //0.1 // 0.3
        $max_devotion_in_group = ceil($percentageDevotionCommittee * $groups[$old_group]['devotion_committee']);
        ...
        do {
            ...
            // Check if the new group has not exceeded the limit
            if (!isset($groups[$old_group]['new_groups'][$new_group])) {
                $groups[$old_group]['new_groups'][$new_group] = ['general' => 0, 'devotion' => 0];
            }
            $general_count = $groups[$old_group]['new_groups'][$new_group]['general'];
            $devotion_count = $groups[$old_group]['new_groups'][$new_group]['devotion'];
        ...
        $groups[$old_group]['new_groups'][$new_group][$is_devotion ? 'devotion' : 'general']++;
